...
#    utils/data/get_segments_for_data.sh data/$mic/$dset
...
if [ $stage -le 7 ]; then
  ...
  steps/decode.sh --nj $nj --cmd "$decode_cmd" --config conf/decode.conf \
    $graph_dir data/$mic/test exp/$mic/tri2/decode_test_${LM}
#  steps/decode.sh --nj $nj --cmd "$decode_cmd" --config conf/decode.conf \
...
if [ $stage -le 9 ]; then
  ...
  steps/decode_fmllr.sh --nj $nj --cmd "$decode_cmd" --config conf/decode.conf \
    $graph_dir data/$mic/test exp/$mic/tri3/decode_test_${LM}
#  steps/decode_fmllr.sh --nj $nj --cmd "$decode_cmd" --config conf/decode.conf \
...
if [ $stage -le 12 ]; then
  steps/score_kaldi.sh --cmd run.pl  data/ihm/test_hires exp/ihm/chain_cleaned/tdnn1j_sp_bi/graph_ami.o3g.kn.pr1-7 \
	  exp/ihm/chain_cleaned/tdnn1j_sp_bi/decode_test
fi
#if [ $stage -le 12 ]; then
